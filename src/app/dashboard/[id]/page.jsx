...
//shows the path to the user's current path
const Breadcrumb = ({ stock }) => (
  ...
  >
    ...
    <span>/</span>
    <span className="text-blue-500">{decodeURIComponent(stock)}</span>
  </motion.div>
);
...
//candlestick chart to display live stockchart with high, low, open and close values.
//the datawould vary after every 5 seconds
//users can select differnt time range to view corresponding

const StockChart = ({ stock }) => {
  const [timeRange, setTimeRange] = useState("5M");
  const [data, setData] = useState(generateRandomData(425371, 5)); //random data starting the 5 values
  ...
  const options = useMemo(
    //React hook as it doesn't require any dependencies to run. Run even if there's no rendering
    () => ({
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
        <div>
          <h2 className="text-2xl font-bold text-white">{decodeURIComponent(stock)}</h2>
...
const generateRandomData2 = (currentValue, points) => {
  const data = [["Time", "nifty", "sensex", "banknifty"]];
  for (let i = 0; i < points; i++) {
    const time = new Date(Date.now() - i * 60000).toLocaleTimeString("en-US", {
      hour: "2-digit",
      minute: "2-digit",
    });    currentValue += Math.random() * 10;
    const cryptoValue = currentValue+ Math.random() * 20 -5 ;
    const bondValue = currentValue+ Math.random() * 40 -5 ;
    data.push([time, currentValue, cryptoValue, bondValue]);
  }

  return data;
};
// it shows the comparison among different assessts
const MultiAssetChart = ({ stock }) => {
  const [timeRange, setTimeRange] = useState("5M");
  const [data, setData] = useState(generateRandomData2(18537, 2)); //random data starting the 5 values
  const [currentValue, setCurrentValue] = useState(18537);
  const [change, setChange] = useState({ value: 0, percentage: 0 });

  useEffect(() => {
    const interval = setInterval(() => {
      const newData = generateRandomData2(
        currentValue,
        getDataPoints(timeRange)
      );
      setData((prevData) => [...prevData, ...newData.slice(1)]);
  ...
  const options = useMemo(
    //React hook as it doesn't require any dependencies to run. Run even if there's no rendering
    () => ({
      curveType: "function", // Makes the lines smooth
      backgroundColor: "transparent",
      chartArea: { width: "90%", height: "80%" },
      hAxis: {
        textStyle: { color: "#9CA3AF" }, //text style is light grey
        baselineColor: "#4B5563",
        gridlines: { color: "transparent" },
        format: "HH:mm",
      },
      vAxis: {
        textStyle: { color: "#9CA3AF" },
        baselineColor: "#4B5563", //base line color is dark grey
        gridlines: { color: "#4B5563" },
      },
      legend: { position: "none" },
  ...
  return (
    ...
    >
      <h3 className="text-xl font-bold text-white mb-4 flex items-center">
        ...
        Top {decodeURIComponent(stock)} Options
      </h3>
  ...
  return (
    <div className="bg-gray-900 min-h-screen text-gray-300">
      ...
      <main className="container mx-auto px-4">
        ...
        <MultiAssetChart stock={params.id} />
